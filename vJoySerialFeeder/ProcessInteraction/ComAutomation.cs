...
namespace vJoySerialFeeder
{
    ...
    public class ComAutomation : IDisposable
    {
        ...
        private ComAutomation()
        {
            ...
            hr = NativeMethods.CreateFileMoniker(PROGID, out IMoniker mon);
            hr = NativeMethods.GetRunningObjectTable(0, out IRunningObjectTable rot);
            rot.Register(1 /*ROTFLAGS_REGISTRATIONKEEPSALIVE*/, comObj, mon);

            // register the object in the Running Object Table by CLSID.
            hr = NativeMethods.RegisterActiveObject(comObj, ref clsid, 0 /*ACTIVEOBJECT_STRONG*/, out int areg);

            // register the object as Out-of-Proces object, which allows go do CreateObject()
            hr = NativeMethods.CoRegisterClassObject(clsid, comObj,
        ...
        internal static class NativeMethods
        {
            ...
            [DllImport("ole32.dll")]
            internal static extern int CreateFileMoniker([MarshalAs(UnmanagedType.LPWStr)] string
               lpszPathName, out IMoniker ppmk);

            [DllImport("ole32.dll")]
            internal static extern int GetRunningObjectTable(uint reserved,
               out System.Runtime.InteropServices.ComTypes.IRunningObjectTable pprot);

            [DllImport("oleaut32.dll")]
            internal static extern int RegisterActiveObject
                ([MarshalAs(UnmanagedType.IUnknown)] object punk,
                ref Guid rclsid,
                uint dwFlags,
                out int pdwRegister);
